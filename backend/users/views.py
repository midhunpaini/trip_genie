...
class FakeDataView(APIView):
    def post(self, request):
        ...
        # # List of possible current locations
        # current_locations = ['New York City', 'San Francisco', 'Chicago', 'Los Angeles', 'Miami']

        # # List of possible destinations
        # destinations = ['London', 'Paris', 'Tokyo', 'Sydney', 'Rome']

        # # List of possible group types
        # group_types = ['Friends', 'Family', 'Couple', 'Solo', 'Business', 'Other']

        # # Create a list of Trip objects
        # trips = []
        # for _ in range(500):
        #     # Generate a random start date between January 1st and today
        #     start_date = date.fromordinal(random.randint(date(2022, 1, 1).toordinal(), today.toordinal()))

        #     # Generate a random end date between the start date and today
        #     end_date = date.fromordinal(random.randint(start_date.toordinal(), today.toordinal()))

        #     trip = Trip(
        #         user=random.choice(all_users),
        #         current_location=random.choice(current_locations),
        #         destination=random.choice(destinations),
        #         destination_id='LA-001',
        #         start_date=start_date,
        #         end_date=end_date,
        #         no_of_people=random.randint(1, 10),
        ...
        # # Bulk create the User objects
        # User.objects.bulk_create(users)

        # dietery_preference_counts = Trip.objects.values('dietery_preference').annotate(count=Count('dietery_preference'))
        # data = []
        # for entry in dietery_preference_counts:
        #     preference = {}
        #     preference['dietery_preference'] = entry['dietery_preference']
        #     preference['count'] = entry['count']
        ...
        for trip in trips:
            trip.country_code = faker.country_code(representation="alpha-3")
            trip.save()
